...
public final class PendingPortalSavedData extends SavedData {
    ...
    public record Entry(String linkId, String dimension, int x, int y, int z, boolean linked, int endX, int endY, int endZ, List<Cell> cells) {
        ...
        public BlockPos worldPos(boolean end, Cell cell) { BlockPos origin = end ? endPos() : pos(); return origin.offset(cell.x(), 0, cell.z()); }
    }
    ...
    public void put(UUID ownerId, UUID linkId, String dimension, BlockPos pos) { entries.put(ownerId.toString(), new Entry(linkId.toString(), dimension, pos.getX(), pos.getY(), pos.getZ(), false, 0, 0, 0, List.of(new Cell(0, 0)))); setDirty(); }
    public boolean link(UUID ownerId, UUID linkId, BlockPos endPos) { Entry entry = entries.get(ownerId.toString()); if (entry == null || !entry.linkId().equals(linkId.toString())) return false; entries.put(ownerId.toString(), new Entry(entry.linkId(), entry.dimension(), entry.x(), entry.y(), entry.z(), true, endPos.getX(), endPos.getY(), endPos.getZ(), entry.cells())); setDirty(); return true; }
    public boolean addCell(UUID ownerId, Cell cell) { Entry entry = entries.get(ownerId.toString()); if (entry == null || !entry.linked() || entry.cells().contains(cell)) return false; List<Cell> cells = new ArrayList<>(entry.cells()); cells.add(cell); entries.put(ownerId.toString(), new Entry(entry.linkId(), entry.dimension(), entry.x(), entry.y(), entry.z(), true, entry.endX(), entry.endY(), entry.endZ(), List.copyOf(cells))); setDirty(); return true; }
    public Optional<OwnedEntry> findLinkedOwned(String dimension, BlockPos pos) { boolean end = Level.END.identifier().toString().equals(dimension); return entries.entrySet().stream().filter(mapEntry -> mapEntry.getValue().linked()).flatMap(mapEntry -> mapEntry.getValue().cells().stream().filter(cell -> mapEntry.getValue().worldPos(end, cell).equals(pos)).map(cell -> new OwnedEntry(UUID.fromString(mapEntry.getKey()), mapEntry.getValue(), cell))).findFirst(); }
